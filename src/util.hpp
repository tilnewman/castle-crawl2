...
namespace util
{
    ...
    [[nodiscard]] inline bool pickNearestVideoMode(sf::VideoMode & videoMode)
    {
        std::vector<sf::VideoMode> possibleVideoModes = sf::VideoMode::getFullscreenModes();

        possibleVideoModes.erase(
            std::remove_if(
                std::begin(possibleVideoModes),
                std::end(possibleVideoModes),
                [&](const sf::VideoMode & vm) {
                    return (vm.bitsPerPixel != videoMode.bitsPerPixel);
                }),
            std::end(possibleVideoModes));
        ...
        auto distance = [&](const sf::VideoMode & A, const sf::VideoMode & B) {
            const int distanceX = abs(static_cast<int>(A.width) - static_cast<int>(B.width));
            const int distanceY = abs(static_cast<int>(A.height) - static_cast<int>(B.height));
        ...
        std::sort(
            ...
            [&](const sf::VideoMode & A, const sf::VideoMode & B) {
                return (distance(videoMode, A) < distance(videoMode, B));
            });

        videoMode = possibleVideoModes.front();
        return true;
    }
