...
namespace castlecrawl
{
    ...
    {
        // write all to spreadsheet sorted by value
        {
            ...
            {
                monsters.emplace_back(pair.first, pair.second);
            }
            ...
            {
                csvFileStream << toString(info.tile) << ',';
                csvFileStream << info.stats.value() << ',';

                std::ostringstream ss;
                ss << std::hex << static_cast<short>(tileImageToChar(info.tile));
                ...
                csvFileStream << info.stats.accuracy << ',';
                csvFileStream << info.stats.luck << ',';

                if (info.stats.is_flying)
                {
                    csvFileStream << "flying";
                }
                csvFileStream << ',';

                if (info.stats.is_undead)
                {
                    csvFileStream << "undead";
                }
                csvFileStream << ',';

                if (info.stats.is_immune_to_nonmagical_weapons)
                {
                    csvFileStream << "immune";
                }
                csvFileStream << ',';

                csvFileStream << info.stats.breathe_fire_attack_ratio << ',';
                csvFileStream << info.stats.poison_attack_ratio << ',';
                csvFileStream << info.stats.acid_attack_ratio << ',';
                csvFileStream << info.stats.spell_attack_ratio << ',';
                csvFileStream << info.stats.break_attack_ratio << ',';
                csvFileStream << info.stats.devour_attack_ratio << ',';

                if (info.stats.is_strong_to_ice_weak_to_fire)
                {
                    csvFileStream << "of ice";
                }
                csvFileStream << ',';

                if (info.stats.is_strong_to_fire_weak_to_ice)
                {
                    csvFileStream << "of fire";
                }
                csvFileStream << ',';

                for (const Spell spell : info.stats.spells)
                {
    ...
    void MonsterStatsDatabase::makeQuickReferenceImage(const Context & t_context) const
    {
        ...
        for (const QuickRefStats & stats : quickRefStats)
        {
            ...
            statsText.setPosition(
                { rowRect.position.x + (rowRect.size.x * 0.45f),
